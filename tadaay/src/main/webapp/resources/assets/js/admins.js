...
$(document).ready(function () {
    var table = $('#adminsDataTable').DataTable({
        "bProcessing": true,
        "bServerSide": true,
        "sAjaxSource": "../showAdmins?userType=A",
    ...
    $("#adminsDataTable_filter input")
            .unbind() // Unbind previous default bindings
            .bind("input", function (e) { // Bind our desired behavior
                // If the length is 3 or more characters, or the user pressed ENTER, search
                if (this.value.length >= 5 || e.keyCode == 13) {
                    // Call the API search function
                    $('#adminsDataTable').DataTable().search(this.value).draw();
                }
                // Ensure we clear the search if they backspace far enough
                if (this.value == "") {
                    $('#adminsDataTable').DataTable().search("").draw();
                }
                return;
            });
    $("#adminsDataTable").on('click', 'input.editor-active', function() {
